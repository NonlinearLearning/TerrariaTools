...
RunReport rsnReport = RunReport.Create(
    workspace.Id,
    rewriteDecision.Id,
    rewritePlan.Id,
    rewriteResult.Id,
...
rsnReport.AttachVerificationEvidence(verificationEvidence.Id);
Assert(rsnReport.VerificationEvidenceId == verificationEvidence.Id, "运行报告应能挂接证据。");
Assert(rsnReport.AuditConclusion == AuditConclusion.RequiresManualReview, "运行报告应保留审计结论。");
...
AssertFileSourceDoesNotContain(
    ...
    "RewriteWorkflowAssemblyInput 应保持输入映射职责边界。");
AssertFileSourceDoesNotContain(
    Path.Combine("src", "Logic", "Workflow", "RewriteWorkflowStageInputs.cs"),
    "BuildEvents(",
    "RewriteWorkflowStageInputs 应保持纯输入模型职责。");
AssertFileSourceDoesNotContain(
    Path.Combine("src", "Logic", "Workflow", "RewriteWorkflowStageInputs.cs"),
    "Propagate(",
    "RewriteWorkflowStageInputs 应保持纯输入模型职责。");
...
AssertFileSourceDoesNotContain(
    ...
    "RewriteWorkflowEventStage 应保持事件职责边界。");
AssertFileSourceContains(
    Path.Combine("src", "Logic", "Workflow", "RewriteWorkflowStageResults.cs"),
    "public RewriteWorkflowArtifacts ToArtifacts(",
    "RewriteWorkflowStageResults 应提供统一 artifacts 回传入口。");
AssertFileSourceContains(
...
AssertFileSourceDoesNotContain(
    Path.Combine("src", "Logic", "Workflow", "RewriteWorkflowStageResults.cs"),
    "BuildEvents(",
    "RewriteWorkflowStageResults 应保持阶段结果职责边界。");
AssertFileSourceDoesNotContain(
    Path.Combine("src", "Logic", "Workflow", "RewriteWorkflowStageResults.cs"),
    "domainEventRecorder",
    "RewriteWorkflowStageResults 应保持阶段结果职责边界。");
