...
- `RuleExecutionPolicy`
- `RuleScope`
- `RuleParticipationMode`
- `RuleConflictMode`
- `RuleFailureMode`
- `RuleSafetyLevel`
- `RuleEvidenceMode`

当前默认规则语义已通过 `WorkspaceContext` 相关收敛落地。

## 2. 当前结论

1. 规则集已经是 Domain 主语义，不再只是应用层配置拼装。
2. 规则执行策略、作用域和证据模式应继续留在 Domain。
3. 后续规则语义扩展优先改 `src/Domain/Rules/**`，再改 `Logic/Workspaces` 和 `Application` 映射层。
