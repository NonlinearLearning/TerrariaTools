...
public sealed class PipelineComponentTests : IDisposable
{
    ...
    public void AnalyzeFromArgs_ForDirectoryDeleteClass_ReportsDiagnosticsForResidualPublicSignatures()
    {
        ...
        var formattedResult = application.FormatResult(result);
        Assert.Contains(
          formattedResult,
          line => line.StartsWith("Diagnostics: ", StringComparison.Ordinal) &&
            !string.Equals(line, "Diagnostics: 0", StringComparison.Ordinal));
        Assert.Contains(
          formattedResult,
          line => line.StartsWith("DIAGNOSTIC Error ", StringComparison.Ordinal) &&
            line.Contains("PlayerInput", StringComparison.Ordinal));
    }
    ...
    public void FormatResult_IncludesCountsDecisionsAndRewrittenSource()
    {
        ...
        Assert.Contains(lines, line =>
          line.StartsWith("PropagatedMarks: ", StringComparison.Ordinal) &&
          int.Parse(line["PropagatedMarks: ".Length..]) >= 1);
        Assert.Contains(lines, line =>
          line.StartsWith("LiftedMarks: ", StringComparison.Ordinal) &&
          int.Parse(line["LiftedMarks: ".Length..]) >= 1);
    ...
    [Fact]
    public void FormatResult_ForDeleteUnreferencedMethods_IncludesRuntimeStats()
    {
        var projectDirectory = Path.Combine(_tempDirectory, "delete-unreferenced-stats-project");
        Directory.CreateDirectory(projectDirectory);
        File.WriteAllText(
          Path.Combine(projectDirectory, "Worker.cs"),
          """
          namespace Demo;
          ...
          """);
        File.WriteAllText(
          Path.Combine(projectDirectory, "Caller.cs"),
          """
          namespace Demo;
        ...
        Assert.Contains(lines, line => string.Equals(line, "ScannedFiles: 2", StringComparison.Ordinal));
        Assert.Contains(lines, line => string.Equals(line, "CandidateMethods: 2", StringComparison.Ordinal));
        Assert.Contains(lines, line => string.Equals(line, "DeletedMethods: 1", StringComparison.Ordinal));
        Assert.Contains(lines, line => line.StartsWith("ElapsedMs: ", StringComparison.Ordinal));
    }
