...
public class FunctionImpactAnalyzerTests
{
    ...
    {
        private static readonly SymbolDependencyGraph EmptyGraph = new(Array.Empty<SymbolDependencyNode>(), Array.Empty<SymbolDependencyEdge>());

        public SymbolDependencyGraph GetWholeGraph() => EmptyGraph;

        public SymbolDependencyGraph GetBackwardSlice(string symbolId) => EmptyGraph;

        public SymbolDependencyGraph GetForwardSlice(IReadOnlyList<string> rootSymbolIds) => EmptyGraph;

        public SymbolDependencyGraph GetBackwardSlice(string symbolId, SymbolDependencyQueryOptions options) => EmptyGraph;

        public SymbolDependencyGraph GetForwardSlice(IReadOnlyList<string> rootSymbolIds, SymbolDependencyQueryOptions options) => EmptyGraph;
    }

    private sealed class StubMethodCallQueryService : IMethodCallQueryService
    {
        private static readonly FunctionDependencyGraph EmptyGraph = new(Array.Empty<FunctionNodeRef>(), Array.Empty<FunctionDependencyEdge>());

        public IReadOnlyList<MemberId> GetCallees(MemberId memberId) => Array.Empty<MemberId>();

        public IReadOnlyList<MemberId> GetCallers(MemberId memberId) => Array.Empty<MemberId>();

        public IReadOnlyList<MemberId> GetReachableMethods(IReadOnlyList<MemberId> rootMemberIds) => rootMemberIds;

        public FunctionDependencyGraph GetWholeGraph() => EmptyGraph;

        public IReadOnlyList<MemberId> GetShortestPath(IReadOnlyList<MemberId> rootMemberIds, MemberId targetMemberId) => Array.Empty<MemberId>();

        public MethodReachabilityExplanation ExplainReachability(MemberId rootMemberId, MemberId targetMemberId) =>
            new(rootMemberId, targetMemberId, false, Array.Empty<MemberId>());
    }

    private sealed class StubDataFlowSummaryService : IDataFlowSummaryService
    {
        public DataFlowSummary Analyze(MemberId memberId) => new(memberId, Array.Empty<string>(), Array.Empty<string>(), Array.Empty<MemberId>());
    }

    private sealed class StubSwitchFlowSummaryService : ISwitchFlowSummaryService
    {
        public IReadOnlyList<SwitchFlowSummary> Analyze(MemberId memberId) => Array.Empty<SwitchFlowSummary>();
    }

    private sealed class StubCallChainAnalysisService : ICallChainAnalysisService
    {
        public IReadOnlyList<CallChainEntry> Parse(string logText) => Array.Empty<CallChainEntry>();

        public CallChainAnalysisSummary Analyze(string logText) => new(0, Array.Empty<MemberId>(), Array.Empty<string>(), Array.Empty<MemberId>());
    }

    private sealed class StubAdvancedAnalysisSummaryService : IAdvancedAnalysisSummaryService
    {
        public AdvancedAnalysisSummary BuildSummary() => new(
            0,
            0,
    ...
    private sealed class StubMemberCleanupQueryService : IMemberCleanupQueryService
    {
        ...
        public IReadOnlyList<MemberId> GetReorderablePublicMethods(string typeId) => Array.Empty<MemberId>();
    }
}
