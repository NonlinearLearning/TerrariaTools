...
public class DataFlowAnalysisTests
{
    ...
    public async Task AnalyzeAsync_ProjectsFieldAndPropertyInitializers()
    {
        ...
        var result = await engine.AnalyzeAsync(new[] { document }, CancellationToken.None);
        var targets = result.View.Targets.ToArray();
        var fieldInitializer = targets.Single(target =>
            target.Target.TargetKind == TargetKind.Statement &&
            target.Target.DisplayText == "private int _seed = 1;");
        var propertyInitializer = targets.Single(target =>
            target.Target.TargetKind == TargetKind.Statement &&
            target.Target.DisplayText == "public int Value { get; set; } = _seed;");
