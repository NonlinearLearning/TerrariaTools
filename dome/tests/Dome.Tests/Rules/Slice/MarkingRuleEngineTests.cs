...
public class MarkingRuleEngineTests
{
    ...
    public async Task DirectiveSeedRule_MarksStatementWithDeleteDirective()
    {
        ...
        var decisions = new MarkingRuleEngine(MarkingRuleRegistry.CreateDefault()).Execute(analysis);
    ...
    [Fact]
    public async Task Execute_PrivatizesPublicMethodWithOnlyInternalReferences()
    {
        var engine = new RoslynAnalysisEngine();
        var analysis = await engine.AnalyzeAsync(
            new[]
        ...
        Assert.Contains(decisions, decision =>
            decision.Target.TargetKind == TargetKind.Method &&
            decision.Target.MemberId.Value == "Sample.Player.Helper()" &&
            decision.Action.Kind == PlanActionKind.ChangeVisibilityToPrivate &&
            decision.Reason.RuleId == "method-privatization");
    }

    [Fact]
    public async Task Execute_DeletesUnusedPrivateFieldAndProperty()
    {
        var engine = new RoslynAnalysisEngine();
        var analysis = await engine.AnalyzeAsync(
            new[]
