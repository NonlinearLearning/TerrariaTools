...
public sealed partial class RoslynRewriteExecutor
{
    ...
    {
        var ruleId = change.Reason is ModelRules.PlanReason typedReason
            ? typedReason.RuleId
            : change.Reason?.GetType().GetProperty("RuleId", BindingFlags.Public | BindingFlags.Instance)?.GetValue(change.Reason)?.ToString()
              ?? "comment-out";
