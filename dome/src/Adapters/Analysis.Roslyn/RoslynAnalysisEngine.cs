...
using TerrariaTools.Dome.Adapters.Analysis.Roslyn.CpgProjection;
...
// 基于 Roslyn 的语法和语义数据构建规范化分析快照。
// 其他层消费生成后的 Core 模型，而不是直接与 Roslyn 交互。
public sealed class RoslynAnalysisEngine : ApplicationAbstractions.IAnalysisEngine
{
    public async Task<ModelAnalysis.AnalysisOutput> AnalyzeAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var context = new BuildContext(sourceSet, compilation, workspaceSemanticModels);
        context.Build(cancellationToken);
        var typeGraph = new ModelAnalysis.TypeDependencyGraph(
            ...
            context.TypeEdges
                ...
                .ToArray());
        var functionIndex = new ModelAnalysis.FunctionIndex(
        ...
        var functionFacts = new ModelAnalysis.FunctionFactsIndex(
            ...
            context.IncomingCallersByMemberId.ToDictionary(
                ...
                StringComparer.Ordinal));
        var cpgProjection = new RoslynCpgProjector().Project(
            sourceSet,
            typeGraph,
            functionIndex,
            functionFacts);
        ...
        var view = new ModelAnalysis.AnalysisResultModel(
            ...
            ModelPrimitives.FunctionGraphMaterialization.WholeProject);
        ...
        var snapshot = new ModelAnalysis.AnalysisExecutionSnapshot(
            ...
            cpgProjection.CodePropertyGraph);
        ...
        var services = new ModelAnalysis.AnalysisServices(
            ...
            new FunctionGraphProvider(snapshot.FunctionIndex, view.FunctionGraph),
    ...
    private static CSharpCompilation CreateCompilation(ModelAnalysis.SourceDocumentSet sourceSet)
    {
    ...
    private sealed class BuildContext
    {
        ...
        public BuildContext(
            ...
            IReadOnlyDictionary<string, SemanticModel> workspaceSemanticModels)
        {
            SourceSet = sourceSet;
            ...
            _documentsByPath = sourceSet.Documents.ToDictionary(
                ...
                document =>
                {
                    var fullPath = Path.GetFullPath(document.SourcePath);
                    if (workspaceSemanticModels.TryGetValue(fullPath, out var workspaceSemanticModel))
                    {
                        return new DocumentContext(document, workspaceSemanticModel.SyntaxTree, workspaceSemanticModel);
                    }
        ...
        public ModelAnalysis.SourceDocumentSet SourceSet { get; }
        ...
        private static ModelAnalysis.AnalysisTarget CreateTarget(
            ...
            new(
                ...
                isHighRisk,
                Array.Empty<ModelAnalysis.DirectiveAction>(),
        ...
        private static string BuildMemberId(ISymbol symbol) =>
            symbol is INamedTypeSymbol namedType
                ? MetadataTypeIdBuilder.Build(namedType)
                : MetadataMemberIdBuilder.Build(symbol).Value;
