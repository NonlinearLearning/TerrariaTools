...
internal static class DeleteSObjectMarkRuleHelpers
{
    public static IEnumerable<MarkRecord> BuildExpressionMarks(
      RuleContext context,
      SyntaxNode root,
      string ruleId,
      IReadOnlyCollection<SyntaxKind> allowedKinds)
    {
        ...
        var markRegionAnalyzer = new MarkRegionAnalyzer();
        foreach (var expression in RuleAnalysisHelpers.EnumerateAllowedExpressions(
                     root,
                     allowedKinds,
                     context.AnalysisContext))
        {
            var markRegion = markRegionAnalyzer.Analyze(expression, context.AnalysisContext);
    ...
    public static IEnumerable<MarkRecord> BuildDefinitionLeftValueMarks(
      RuleContext context,
      SyntaxNode root,
      string ruleId)
    {
    ...
    private static bool HasRootedObjectAncestorInRegion(
      RuleContext context,
      ExpressionSyntax expression,
      IReadOnlyList<string> targetNames,
      MarkCodeRegion markRegion)
    {
    ...
    private static bool IsRootedAtTarget(
      RuleContext context,
      ExpressionSyntax expression,
      IReadOnlyList<string> targetNames)
    {
    ...
    private static bool ReferencesTarget(
    ...
    private static bool ReferencesTargetMemberBinding(
      RuleContext context,
    ...
    private static bool IsTargetInstanceReference(
    ...
    private static bool HasCpgNodeInRegion(
      RuleContext context,
      ExpressionSyntax expression,
      MarkCodeRegion markRegion)
    {
        var filePath = expression.SyntaxTree.FilePath;
        if (string.IsNullOrWhiteSpace(filePath))
        {
            return false;
        }

        return context.Graph.Nodes.Any(node =>
          !node.IsImplicit &&
          string.Equals(node.FilePath, filePath, StringComparison.Ordinal) &&
          node.SpanStart == expression.SpanStart &&
          node.SpanEnd == expression.Span.End &&
          node.SpanStart >= markRegion.Span.Start &&
          node.SpanEnd <= markRegion.Span.End);
    }
}
