...
public sealed class RoslynCpgStructureViewBuilder
{
    ...
    /// </summary>
    private static IReadOnlyList<RoslynCpgNode> ResolveGraphNodesInside(
    ...
    /// </summary>
    private static void AddShortestConnectingPaths(
        RoslynCpgGraph graph,
        IReadOnlyList<IReadOnlyList<RoslynCpgNode>> fragmentNodeSets,
        ISet<string> selectedNodeIds,
        ISet<RoslynCpgEdge> selectedEdges)
    {
    ...
    /// </summary>
    private static Dictionary<string, List<(string NeighborId, RoslynCpgEdge Edge)>> BuildUndirectedAdjacency(
    ...
    /// </summary>
    private static IReadOnlyList<RoslynCpgEdge>? FindShortestPath(
        IReadOnlyDictionary<string, List<(string NeighborId, RoslynCpgEdge Edge)>> adjacency,
        ISet<string> sourceNodeIds,
        ISet<string> targetNodeIds)
    {
    ...
    /// </summary>
    private static IReadOnlyList<RoslynCpgEdge> ReconstructPath(
        IReadOnlyDictionary<string, (string PreviousId, RoslynCpgEdge Edge)> previous,
        string targetNodeId)
    {
    ...
    private static void AddNeighbor(
        IDictionary<string, List<(string NeighborId, RoslynCpgEdge Edge)>> adjacency,
        string sourceId,
        string targetId,
        RoslynCpgEdge edge)
    {
    ...
    private static RoslynCpgNode SelectRootNode(SyntaxNode firstFragment, IReadOnlyList<RoslynCpgNode> nodes)
    {
        return nodes
            ...
            .OrderBy(node => node.Kind == Contracts.RoslynCpgNodeKind.SyntaxNode ? 0 : 1)
            .ThenBy(node => node.Id, StringComparer.Ordinal)
            .FirstOrDefault()
            ?? nodes.First();
    }
