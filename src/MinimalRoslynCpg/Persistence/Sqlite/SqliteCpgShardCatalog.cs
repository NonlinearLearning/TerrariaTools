...
public sealed class SqliteCpgShardCatalog : ICpgShardCatalog
{
  ...
  public async Task StageAsync(
    ...
    CancellationToken cancellationToken)
  {
    await StageBatchAsync(buildId, new[] { new CpgCatalogPublication(lease, shard) }, cancellationToken);
  }
    ...
    CancellationToken cancellationToken)
  {
    await StageBatchAsync(
      buildId,
      new[] { new CpgCatalogPublication(lease, shard, reusableKey) },
      cancellationToken);
  }
    ...
    CancellationToken cancellationToken)
  {
    ...
    await using var connection = await OpenAsync(cancellationToken);
    await StageBatchAsync(connection, buildId, publications, commandCache: null, cancellationToken);
  }
    ...
    CancellationToken cancellationToken)
  {
    ...
    {
      await StageOneAsync(connection, transaction, buildId, publication, commandCache, cancellationToken);
    }
  ...
  private static Task ExecuteStageAsync(
    SqliteCpgCatalogCommandCache? commandCache,
    SqliteConnection connection,
    SqliteTransaction transaction,
    string sql,
  ...
  {
    return commandCache is null
      ? ExecuteAsync(connection, transaction, sql, cancellationToken, values)
      : commandCache.ExecuteAsync(transaction, sql, cancellationToken, values);
  }
    ...
    CancellationToken cancellationToken)
  {
    ...
    for (var start = 0; start < rows.Count; start += maxRowsPerStatement)
    {
      ...
      {
        await ExecuteValuesAsync(connection, transaction, sql, values, cancellationToken);
      }
      else
      {
        await commandCache.ExecuteValuesAsync(transaction, sql, values, cancellationToken);
      }
    }
  }

  private static async Task ExecuteValuesAsync(
    SqliteConnection connection,
    SqliteTransaction transaction,
    string sql,
  ...
  private static Task RefreshPhysicalShardReferencesAsync(
    SqliteConnection connection,
    CancellationToken cancellationToken)
  {
    return RefreshPhysicalShardReferencesAsync(connection, transaction: null, cancellationToken);
  }

  private static async Task RefreshPhysicalShardReferencesAsync(
    SqliteConnection connection,
    SqliteTransaction? transaction,
    CancellationToken cancellationToken)
  {
    await ExecuteAsync(
      connection,
      transaction,
      "DELETE FROM physical_shard_references;",
      cancellationToken);
    await ExecuteAsync(
      connection,
      transaction,
      ...
      SELECT s.shard_path, s.shard_hash, s.byte_length, COUNT(*)
      FROM session_shards s JOIN build_sessions b ON b.build_id = s.build_id
      WHERE b.status = 'Complete'
      GROUP BY s.shard_path, s.shard_hash, s.byte_length;
      """,
      cancellationToken);
  }
  ...
  public async Task CompleteBuildAsync(string buildId, CancellationToken cancellationToken)
  {
    ...
    await using var connection = await OpenAsync(cancellationToken);
    await using var command = connection.CreateCommand();
    command.CommandText = "UPDATE build_sessions SET status = 'Complete', completed_at_utc = $completedAt WHERE build_id = $buildId AND status = 'Building';";
    ...
    manifest.CommandText = """
      ...
      SELECT $buildId, $completedAt, COUNT(*), COALESCE(SUM(byte_length), 0)
      FROM session_shards WHERE build_id = $buildId;
      """;
    manifest.Parameters.AddWithValue("$buildId", buildId);
    manifest.Parameters.AddWithValue("$completedAt", DateTimeOffset.UtcNow.ToString("O"));
    await manifest.ExecuteNonQueryAsync(cancellationToken);
    await RefreshPhysicalShardReferencesAsync(connection, cancellationToken);
  }
    ...
    CancellationToken cancellationToken)
  {
    ...
    await using (var transaction = (SqliteTransaction)await connection.BeginTransactionAsync(cancellationToken))
    {
      ...
      await RefreshPhysicalShardReferencesAsync(connection, transaction, cancellationToken);
      await transaction.CommitAsync(cancellationToken);
    }
  ...
  private static async Task ExecuteAsync(SqliteConnection connection, SqliteTransaction? transaction, string sql, CancellationToken cancellationToken, params object?[] values)
  {
    await using var command = connection.CreateCommand();
    command.Transaction = transaction;
    command.CommandText = sql;
...
internal sealed class SqliteCpgCatalogCommandCache : IAsyncDisposable
{
  ...
  internal async Task ExecuteValuesAsync(
    SqliteTransaction transaction,
    string sql,
    IReadOnlyList<object?> values,
    CancellationToken cancellationToken)
