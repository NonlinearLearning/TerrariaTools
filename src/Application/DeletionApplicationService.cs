...
public sealed class DeletionApplicationService
{
  ...
  private readonly IReadOnlyList<IDeletionRule> _rules;
  private readonly IMarkingEngine _markingEngine;
  private readonly IPropagationEngine _propagationEngine;
  private readonly RuleDecisionEngine _decisionEngine;
  private readonly PrototypeRewriter _rewriter;

  public DeletionApplicationService(IReadOnlyList<IDeletionRule> rules)
  {
    ...
    IReadOnlyDictionary<string, string> options)
  {
    ...
    var decisions = _decisionEngine.Decide(seedMarks, propagatedMarks);
    ...
    IReadOnlyDictionary<string, string> options)
  {
    ...
    var ruleContext = new RuleContext(graph, semanticModel, root, options);
