...
### **2.1. 加载与环境 (Services)**
- **IWorkspaceLoader**: 定义了解决方案加载与文件操作的抽象接口，支持依赖注入。
- **WorkspaceLoader**: `IWorkspaceLoader` 的具体实现，负责 MSBuild 环境的初始化、解决方案 (`.sln`) 和项目 (`.csproj`) 的加载。
- **关键技术**: `Microsoft.CodeAnalysis.MSBuild.MSBuildWorkspace`。
- **功能**: 管理 `SemanticModel` 的生命周期，提供跨项目的符号查找能力，并封装了文件系统的读写操作以支持 Mock 测试。

### **2.2. 配置管理 (Configuration)**
- **RefactoringSettings**: 强类型的配置类，映射自 `appsettings.json`。
- **参数支持**:
    - `IgnoredFiles`: 允许用户指定不需要重构的文件列表。
    - `Parallelism`: 控制并行分析的并发度。
    - `EnableDryRun`: 开关，用于在不修改磁盘文件的情况下预览重构结果。

### **2.3. 重写引擎 (RewriteCodeExpressions)**
...
### **2.5. 深度分析模块 (Analysis)**
- **DependencyGraph**: 核心图引擎。利用 Tarjan 算法解决循环依赖，并提供拓扑排序能力。
- **CodeDependencyAnalyzer**: 语义扫描器。递归遍历方法体，识别所有符号级的依赖关系。
- **MemberSlicingRewriter**: 精细化重写器。执行成员级的代码切分与移除。
- **ShadowClassGenerator**: 影子类引擎。协调分析与重写，生成最小化源码集。

### **2.6. 动态分析模块 (DynamicAnalysis)**
- **FieldAccessTracer**: 运行时探测器。基于 Harmony AOP 动态拦截方法，记录实际的字段访问轨迹。

### **2.7. 行为保证 (ConsistentBehaviorGuarantee)**
...
graph TD
    ...
    F --> G[向上结构传播]
    G --> H[第二阶段: 语法重写与占位符填充]
    H --> L[IWorkspaceLoader (文件保存/DryRun)]
    L --> M[磁盘/报告]
    H --> I[差分测试验证]
```
...
├── Configuration/            # 配置管理 (RefactoringSettings, appsettings.json)
├── Services/                 # 核心服务 (IWorkspaceLoader, WorkspaceLoader)
