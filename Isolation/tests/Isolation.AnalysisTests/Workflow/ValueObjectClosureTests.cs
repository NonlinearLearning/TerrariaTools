...
public sealed class ValueObjectClosureTests
{
    ...
    [Fact]
    public void Propagation_value_objects_normalize_inputs_and_keep_string_projection()
    {
        SliceBoundary boundary = new("  EntryClosure  ", SliceDirection.Bidirectional, 3, true);
        ReferenceMapping mapping = new("  PlayerTools.Entry  ", "  PlayerTools.Helper  ");
        ClosureRoot root = new("  PlayerTools  ", "  Entry  ");

        Assert.Equal("EntryClosure", boundary.BoundaryName);
        Assert.Equal(SliceDirection.Bidirectional, boundary.Direction);
        Assert.Equal(3, boundary.MaxDepth);
        Assert.True(boundary.IncludeExternalReferences);

        Assert.Equal("PlayerTools.Entry", mapping.SourceReference);
        Assert.Equal("PlayerTools.Helper", mapping.TargetReference);
        Assert.Equal("PlayerTools", root.ClassName);
        Assert.Equal("Entry", root.MemberName);
    }
}
