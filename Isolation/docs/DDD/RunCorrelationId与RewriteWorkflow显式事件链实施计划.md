# RunCorrelationId 与 RewriteWorkflow 显式事件链实施现状

## 1. 当前代码事实

当前 `RewriteWorkflow` 事件链已经显式携带运行相关上下文，关键对象包括：

- `RewriteWorkflowCorrelationResolver`
- `RewriteWorkflowAssemblyInput`
- `DomainEventContext`
- `DomainEventEnvelope`
- 各阶段 `*DomainEventPublishInput`
...
1. `RunCorrelationId` 已经不再是隐式约定，而是工作流主链显式输入的一部分。
2. 当前最值钱的后续动作是继续守护“跨 run 不串线”，而不是再造新的 correlation 抽象。
