...
- For Codex native child agents / subagents in this repository, use model `gpt-5.4` with `medium` reasoning effort as the default execution configuration.
- When task-specific instructions do not explicitly require another model profile, keep subagent invocations aligned to `gpt-5.4 medium`.
