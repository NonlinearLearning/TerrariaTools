...
internal sealed class PrimitiveBuilder
{
    ...
    {
        ArgumentNullException.ThrowIfNull(node);
        node.SetProperty("Line", lineSpan.StartLinePosition.Line + 1);
        node.SetProperty("Column", lineSpan.StartLinePosition.Character + 1);
    }
    ...
    public OperationId CreateOperationId(SyntaxNode syntaxNode)
    {
        ...
        string filePath = string.IsNullOrWhiteSpace(lineSpan.Path) ? "<memory>" : lineSpan.Path;
        return new OperationId($"{filePath}:{syntaxNode.SpanStart}:{syntaxNode.RawKind}");
    }
        ...
        CpgNode fileNode)
    {
        ...
        node.SetProperty("DeclaredSymbolId", RoslynSymbolFormatter.GetSymbolId(declaredSymbol)?.Value);
        node.SetProperty("AstParentId", astParentId);
        node.SetProperty("FileName", GetStringProperty(fileNode, "FileName"));
    ...
    public CpgNode CreateControlNode(SyntaxNode syntaxNode, long astParentId, string controlType)
    {
        ...
        controlNode.SetProperty("Name", controlType);
        controlNode.SetProperty("ControlStructureType", controlType);
        controlNode.SetProperty("AstParentId", astParentId);
