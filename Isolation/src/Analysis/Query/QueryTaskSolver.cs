...
/// 负责执行单个查询任务。
///
/// 这个类型对应 Joern `TaskSolver.scala` 的简化版：
...
public sealed class QueryTaskSolver
{
    ...
    {
        return graph.GetIncomingEdges(currentNodeId, CpgEdgeKind.ReachingDef)
            .Concat(graph.GetIncomingEdges(currentNodeId, CpgEdgeKind.ParameterLink));
    }
}
