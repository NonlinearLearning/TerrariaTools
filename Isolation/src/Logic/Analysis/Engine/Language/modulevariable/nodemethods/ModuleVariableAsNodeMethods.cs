...
namespace Logic.Analysis.Engine.Language.ModuleVariable.NodeMethods;

/// <summary>
/// 对应 Joern semanticcpg/language/modulevariable/nodemethods/ModuleVariableAsNodeMethods.scala。
///
/// 该文件提供 C# 查询 DSL 的一个命名入口，避免调用方直接操作字符串属性和边集合。
/// </summary>
public static class ModuleVariableAsNodeMethods
{
    /// <summary>
    /// 从当前遍历中选择该文件负责的节点集合。
    /// </summary>
    ...
    public static Traversal Select(Traversal traversal)
    {
        ...
        return CpgNodeKind.Method == CpgNodeKind.Unknown ? traversal : traversal.OfKind(CpgNodeKind.Method);
    }
